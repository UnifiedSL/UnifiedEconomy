namespace UnifiedEconomy.Command.Admin
{
    ...
    public class EcoTakeCommand : ICommand
    {
        ...
        /// <inheritdoc/>
        public string Description { get; } = "Takes the specified player/s the amount of money.";

        public string Permission { get; } = string.IsNullOrEmpty(UEMain.Singleton.Config.Economy.PermissionForAdminCommand) ? string.Empty : UEMain.Singleton.Config.Economy.PermissionForAdminCommand + ".take";
        ...
        public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
        {
            ...
            players.ForEach(player => player.AddBalance(-Math.Abs(duration)));

            response = $"You have taken {duration}$ to {(players.Count == 1 ? players.FirstOrDefault().Nickname : (players.Count + " players"))}!";
            return true;
        }
    }
}
